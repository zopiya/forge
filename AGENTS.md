...
- `.github/workflows/lint.yml` typechecks `.pi/extensions/*.ts` (`cd .pi && bun run typecheck` — same command locally; the `package.json`/`tsconfig.json`/`bun.lock` behind it live in `.pi/`, not the repo root, on purpose — see `docs/design.md` §18), and syntax/shape-checks scripts, JSON, and prompt/skill frontmatter. This is mechanical hygiene, not a substitute for actually running something before calling it done — see `docs/design.md` §13.
