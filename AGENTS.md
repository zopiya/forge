...
Beyond `subagent`, `.pi/extensions/` has seven more vendored example extensions from pi upstream (feature/UX additions, not guardrails — Forge runs in a container/Codespaces already, see the assumptions above). Rationale and any deviations from upstream are in `docs/design.md` §9.
