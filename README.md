...
- `.pi/extensions/{plan-mode,custom-footer,session-name,notify,handoff,trigger-compact}` — six more of pi's own example extensions, vendored the same way as `subagent`. Feature/UX additions, not guardrails — see `docs/design.md` §9 for what each does and why, and AGENTS.md for the commands they add.
