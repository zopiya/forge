...
- `.pi/extensions/{plan-mode,custom-footer,session-name,notify,handoff,trigger-compact,questionnaire.ts}` — seven more of pi's own example extensions, vendored the same way as `subagent`. Feature/UX additions, not guardrails — see `docs/design.md` §9 for what each does and why, and AGENTS.md for the commands they add. `questionnaire.ts` is the odd one out — it adds no command of its own, it just registers the `questionnaire` tool that `plan-mode` already assumes exists.
