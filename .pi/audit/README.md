...
the generic, goal-agnostic loop engine at `.pi/scripts/loop.sh`. It sets up
the audit-specific state (a dedicated git worktree + branch via
`.pi/scripts/worktree.sh`, dirty-tree fail-closed checks via
...
schedule — not an audit — call `.pi/scripts/loop.sh` directly with a
different `--prompt`; see that file's header for the full option list.

The audit branch runs in its own worktree (sibling of this checkout, see
`.pi/skills/worktree/SKILL.md`) rather than switching branches in whatever
checkout happens to be current — an unattended midnight process must never
leave your own working directory on a different branch. `run.sh` creates it,
reuses it on a same-day rerun, and removes it automatically once the loop
finishes clean; a dirty finish leaves it in place for inspection instead.

## Files

| File | Committed? | Purpose |
|---|---|---|
| `run.sh` | Yes | Audit-domain wrapper: worktree+branch setup via `.pi/scripts/worktree.sh`, dirty-tree hooks, final summary notification. Delegates the loop itself to `.pi/scripts/loop.sh`. |
...
| `run.log` | No (gitignored, `*.log`) | Plain timestamped operational log — written by `.pi/scripts/loop.sh` (round starts/stops, exit codes, durations) and by `run.sh`'s own worktree/dirty-check lines. |
...
dry run first, in a throwaway worktree so a bug in `run.sh` itself can't touch
real work — note this outer worktree is for isolating the *test of `run.sh`*;
it's separate from the audit-branch worktree `run.sh` creates for itself
automatically once it starts:

```
.pi/scripts/worktree.sh add ../forge-audit-dryrun test/audit-dryrun
...
immediately. Confirm: `run.sh` creates its own `chore/nightly-audit-<date>`
worktree+branch (check `git worktree list` from `../forge-audit-dryrun`),
`log.md` gets a real entry, `run.log` has round-by-round timestamps, a push
notification arrives, the audit worktree is removed automatically once the
loop finishes clean, and the loop stops at 2 rounds or ~2 minutes, whichever
...
uncommitted change in the audit worktree before starting, confirm `run.sh`
refuses to run and leaves that worktree in place instead of removing it)
before ever trusting a real overnight run. Remove the outer dry-run worktree
afterward: `.pi/scripts/worktree.sh remove ../forge-audit-dryrun --delete-branch`.
